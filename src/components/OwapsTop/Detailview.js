...
const DetailView = (props) => {
	...
	if (props.details === true) {
		...
		application_vulnerable = (
			<GetListItems item={item_obj.application_vulnerable} />
		);
	...
	return (
		<Table striped bordered hover style={{ backgroundColor: "white" }}>
			<thead>
						...
						<Table striped bordered hover style={{ backgroundColor: "white" }}>
							<thead>
								<tr style={{ backgroundColor: "whitesmoke" }}>
									<th colSpan="2">Threat Agents / Attack Vectors</th>
									<th colSpan="2">Security Weakness</th>
									<th colSpan="2">Impacts</th>
								</tr>
								<tr>
									<td>App. Specific</td>
				...
				<tr>
					<td style={{ width: "50%" }}>
						Is the Application Vulnerable? <hr /> {application_vulnerable}
					</td>
					<td>
						How to Prevent <hr /> {prevent}
					</td>
				</tr>
				<tr>
					<td>
						Example Attack Scenarios <hr /> {attack}
					</td>
					<td>
						References <hr /> {url}
					</td>
